...
# The whole game is always simulated on this fixed-size "landscape"
# surface -- paddles as vertical bars left/right, ball travelling mostly
# left/right. For Player vs AI we display it rotated 90 degrees into a
# portrait window (matching a vertical soccer pitch); for Player vs
# Player it's blitted straight into a landscape window. Nothing about
# the physics below needs to know which one is happening.
...
def start_serve(state):
    """Begins a non-blocking countdown before the ball launches."""
    state["serving"] = True
    state["serve_start_time"] = pygame.time.get_ticks()
    state["last_speed_bump"] = pygame.time.get_ticks()  # don't let a bump fire mid-countdown
    ball.center = (LOGICAL_WIDTH / 2, LOGICAL_HEIGHT / 2)
    state["ball_trail"].clear()


def update_serve_countdown(state):
    """Returns the number to display (3, 2, 1) or None once serve should launch."""
...
def ball_animation(state):
    ...
    # trail effect (the smooth "motion blur" look)
    ...
    if hit_player or hit_opponent:
        if hit_player:
            ...
            ball.right = player.left  # nudge out so it doesn't get stuck re-colliding
    ...
    # gentle speed ramp every ~6 seconds, capped so it can't run away
    now = pygame.time.get_ticks()
...
def apply_paddle_hit(state, paddle, moving_up, moving_down):
    """Makes the bounce direction depend on:
    1) where along the paddle the ball made contact (top vs bottom vs center), and
    2) whether the paddle was actively moving up/down at the moment of the hit.
    This is what real Pong-style physics does -- a paddle in motion "carries"
    the ball in that direction, and off-center hits create sharper angles.
    """
...
# ---------- In-game drawing (all onto the fixed-size logical surface) ----------

def draw_soccer_ball(surface, rect, rotation_deg):
    """A simplified, rotating soccer-ball skin: white sphere, a black
    centre pentagon, and five black patches spaced evenly around it. The
    whole pattern spins with rotation_deg so the ball visibly rolls."""
    center = rect.center
    radius = rect.width / 2
...
def draw_net_strip(surface, x0, x1):
    """A crosshatched goal net confined to a thin vertical strip at the
    edge of the pitch, with a post along each side of the strip."""
...
def draw_pitch(surface):
    """The playing field: a green pitch with a halfway line, centre
    circle, two penalty boxes, and a goal net behind each scoring edge.
    Drawn in logical (landscape) space; for Player vs AI the whole frame
    gets rotated afterwards so this becomes a vertical pitch, matching a
    real soccer pitch viewed end-to-end."""
...
# ---------- Overlay drawing (onto the real, on-screen surface, always upright) ----------
...
def handle_events(state):
    """Returns (running, next_app_state).

    Control scheme depends on pitch orientation:
    - Player vs AI (vertical pitch): the human paddle is the horizontal
      bar at the *bottom* of the portrait screen, moved with LEFT/RIGHT.
      (Internally that's still the "player" rect sliding along its one
      axis -- only the on-screen rotation makes it look horizontal.)
    - Player vs Player (horizontal pitch): unchanged, UP/DOWN and W/S.
    """
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if not state["game_over"] and not state["paused"]:
                if state["opponent_is_ai"]:
                    # vertical pitch: LEFT/RIGHT are remapped onto the
                    # logical up/down axis so the rotated paddle slides
                    # the intuitive direction on screen.
...
def set_window_for_mode(vertical):
    """Portrait window for Player vs AI (matches a vertical soccer
    pitch), landscape window for Player vs Player / menus."""
    if vertical:
        return pygame.display.set_mode((LOGICAL_HEIGHT, LOGICAL_WIDTH))
    return pygame.display.set_mode((LOGICAL_WIDTH, LOGICAL_HEIGHT))
...
def pong1():
    ...
    screen = set_window_for_mode(False)  # menus are always landscape
    ...
    while running:
        if app_state == "menu":
            ...
            # short, decaying screen shake on scoring
            offset = (0, 0)
            ...
            if state["opponent_is_ai"]:
                # rotate the whole landscape frame 90 deg clockwise so it
                # fills the portrait window: left edge (AI) -> top,
                # right edge (player) -> bottom, matching a vertical pitch.
                rotated = pygame.transform.rotate(game_surface, -90)
                screen.blit(rotated, offset)
            else:
                screen.blit(game_surface, offset)

            # overlays drawn last, directly on the real screen, so text
            # always reads upright regardless of pitch orientation
